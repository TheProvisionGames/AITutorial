...
def update_data_labels(gameDisplay, dt, game_time, num_iterations, font): #This is where you define the text that will get on screen
    y_pos = 10
    gap = 20
    x_pos = 10
    y_pos = update_label(round(1000 / dt, 2), 'FPS', font, x_pos, y_pos + gap, gameDisplay) #show frames per second
    y_pos = update_label(round(game_time/1000, 2), 'Game time', font, x_pos, y_pos + gap, gameDisplay) #show game time
    y_pos = update_label(num_iterations, 'Iterations', font, x_pos, y_pos + gap, gameDisplay) #show number of iterations
